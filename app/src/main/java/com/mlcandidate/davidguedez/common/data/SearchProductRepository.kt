...
class SearchProductRepository @Inject constructor(
    ...
    override suspend fun search(siteId: String,searchQuery: String): List<Product> {
        try {
            val searchResult = apiSearchMapper.mapToDomain(apiProductService.searchProduct(siteId,searchQuery))
